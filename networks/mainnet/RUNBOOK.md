...
$BIN genesis add-genesis-account <address> <amount>amantra --home /tmp/mainnet-genesis
# amantra = 18 decimals (1 mantra = 1 followed by 18 zeros amantra). This is
...
# 'amantra'/'mantra'/18 below must match whatever denom actually ended up
# in genesis-params.json above.
patch.setdefault('app_state', {}).setdefault('bank', {})['denom_metadata'] = [{
    'description': 'The native staking and governance token.',
    'denom_units': [{'denom': 'amantra', 'exponent': 0}, {'denom': 'mantra', 'exponent': 18}],
    'base': 'amantra', 'display': 'mantra', 'name': 'Mantra', 'symbol': 'MANTRA',
}]
def merge(a, b):
    for k, v in b.items():
...
timeout 10 $BIN start --home /tmp/smoke-test --minimum-gas-prices 0amantra || true
