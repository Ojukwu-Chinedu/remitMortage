...
- **`app/ibc_middleware/migrate_uom.go`** — MANTRA-specific one-time `uom`→`amantra` IBC-voucher migration middleware, still wired into the IBC transfer stack. Inert for Ark (no chain will ever send Ark a `uom` voucher), but not one of the three modules named for removal, so left in place. Candidate for a follow-up cleanup PR.
...
- **Two independent hardcoded real MANTRA mainnet addresses found and removed** this session: `x/tax`'s `DefaultMcaAddress` (removed with the module) and the v8.4.0 upgrade handler's blacklisted account (removed with `x/sanction`). Both were genuine production MANTRA data with no place in Ark's codebase. Worth a final grep sweep (`grep -rn "mantra1[a-z0-9]\{30,\}"`) before any real mainnet genesis day, in case a third instance exists somewhere not touched by this session's specific module-removal work.
